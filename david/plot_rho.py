...
def _obj_func_lo(time,sig_c,sig_l,sig_0,tau):
    return sig_c + sig_l*time + sig_0 * np.exp(time/tau)

def _obj_func_mid(time,sig_hi, sig_0, tau, sig_lo):
    return sig_hi - sig_0 * np.exp(-time/tau) + sig_lo
...
def fit_lo(time,conductivity,lims):
    ...
    params = [1,0.001,50,0.2]
    popt, pcov = curve_fit(f,_t,_c,p0=params)
    
    perr = np.sqrt(np.diag(pcov))
    
    sig_c, sig_l, sig_0, tau = popt
    err_c, err_l, err_0, err_tau = perr
    fit = f(_t,*popt)
    
    
    return _t, _t0, sig_c, sig_l, sig_0, tau, fit, err_c, err_0, err_tau

# --------------------------------------------------------------------------------------------------

def fit_mid(time,conductivity,lims,buffer=20):
    
    """
    fit exponential decay constant for current: add a linear term for early
    stage of incubation
        
    sig = sig_hi - sig_0 * exp(-t/tau) + sig_lo
    """
    ...
    _inds = np.flatnonzero(_t > _t[_max]) #np.flatnonzero((_const - _c)/_const > 0.1 )
    
    _inds = _inds[buffer:-buffer]
    ...
    sig_hi, sig_0, tau, sig_lo = popt
    err_hi, err_0, err_tau, err_lo = perr
    fit = f(_t,*popt)

    return _t, _t0, sig_hi, sig_0, tau, sig_lo, fit, err_hi, err_0, err_tau, err_lo

# --------------------------------------------------------------------------------------------------

def fit_hi(time,conductivity,lims,buffer=20):
    ...
    params = [60,50,5,1,-0.001]
    popt, pcov = curve_fit(f,_t,_c,p0=params)
    
    perr = np.sqrt(np.diag(pcov))
    
    sig_hi, sig_0, tau, sig_lo, sig_lin = popt
    err_hi, err_0, err_tau, err_lo, err_lin = perr
    fit = f(_t,*popt)
    
    return _t, _t0, sig_hi, sig_0, tau, sig_lo, sig_lin, fit, err_hi, err_0, err_tau, err_lo, err_lin
...
def plot_and_fit(file_name,sample_len,sample_area,xlims,ylims,which):
    ...
    if which == 'lo':
        t, t0, sig_c, sig_l, sig_0, tau, fit, err_c, err_0, err_tau = \
            fit_lo(time,conductivity,xlims)
    elif which == 'mid':
        t, t0, sig_hi, sig_0, tau, sig_lo, fit, err_hi, err_0, err_tau, err_lo = \
            fit_mid(time,conductivity,xlims)
    elif which == 'hi':
        t, t0, sig_hi, sig_0, tau, sig_lo, sig_lin, fit, err_hi, err_0, err_tau, err_lo, err_lin = \
            fit_hi(time,conductivity,xlims)
        
        _label = f'{temp}_{environ}'
        _m = f"'{_label}':dict(t0={t0:.3f},sig_hi={sig_hi:.3f},sig_0={sig_0:.3f},tau={tau:.3f},\n"
        _m += f" sig_lo={sig_lo:.3f},sig_lin={sig_lin:.3f},err_tau={err_tau:.3f})\n"
        print(_m)
    ...
    ax.plot(t+t0,fit,c='k',ls=(0,(4,1,1,1)),lw=2)
