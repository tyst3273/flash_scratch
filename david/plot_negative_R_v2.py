...
fig, ax = plt.subplots(2,1,figsize=(4,4),gridspec_kw={'hspace':0.2,'wspace':0.2}) 
# twin_ax = ax.twinx()
...
E *= 10
ax[1].plot(j,E,lw=1,marker='^',ms=5,c='m',label=r'TiO$_2$')
#for ii in range(j.size):
#    ax[1].annotate(rf'{ii+1}',xy=(j[ii]+1,E[ii]+2),fontsize='medium',xycoords='data')
...
for _ax in axes:
    ...
    _ax.legend(frameon=False,fontsize='large')

ax[0].set_xlim(-20,825)
ax[1].set_xlim(17,55)

ax[0].set_ylim(20,110.5)
ax[1].set_ylim(190,400)
...
#ax[0].annotate(rf'Si - Fan on',xy=(0.5,0.9),
#                   xycoords='axes fraction',fontsize='large')
#ax[1].annotate(rf'TiO$_2$',xy=(0.75,0.9),
#                   xycoords='axes fraction',fontsize='large')

ax[0].annotate('(a)',xy=(0.01,0.05),
                   xycoords='axes fraction',fontsize='large')
ax[1].annotate('(b)',xy=(0.01,0.05),
                   xycoords='axes fraction',fontsize='large')

ax[1].set_xlabel(r'Current density [mA/mm$^2$]',fontsize='large',labelpad=1)

fig.supylabel(r'Electric field [V/cm]',fontsize='large',x=-0.025)
#ax[1].set_ylabel(r'Electric field [V/cm]',fontsize='large',labelpad=5)

# fig_name = f'TiO2_{temp}_C_{environ}_{which}_fit.png'
plt.savefig('iv_hysteresis_new.pdf',dpi=300,bbox_inches='tight')
